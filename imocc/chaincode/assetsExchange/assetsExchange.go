...
type AssetExchangeCC struct{}

const (
	originOwner = "originOwnerPlaceholder"
)

//实际存储是通过字符数组存储的,所以需要将实体进行序列化后再进行存储
//用户
type User struct {
	Name string `json:"name"` //messagepack || protobuf
	Id string	`json:"id"`
	//Assets map[string]string `json:"Assets"``//资产 key:id , value:name
...
type AssetHistory struct {
	...
	CurrentOwnerId string `json:"current_owner_id"` //变更后当前的拥有者Id
}
...
func userRegister(stub shim.ChaincodeStubInterface,args []string) pb.Response{
	...
	//如果读取没有错误并且返回的userBytes长度不为0则表示正确
	...
	fmt.Println("user successful registered!")
	fmt.Println("hahaha im testing registered dev")
	//成功返回
...
func AssetEnroll(stub shim.ChaincodeStubInterface,args []string) pb.Response {
	...
	userBytes,err = json.Marshal(user)
	if err != nil {
		return shim.Error(fmt.Sprintf("marshal user error : %s",asset))
	}
...
//资产转让
func AssetChange(stub shim.ChaincodeStubInterface,args []string) pb.Response {
	if len(args) != 3 {
		return shim.Error("not enough args")
	}
	// 套路2：验证参数的正确性
	...
	//套路4:写入状态
	//1.原始拥有者删除资产id,2.新拥有者加入资产3.资产变更记录
...
func queryAssetHistory(stub shim.ChaincodeStubInterface,args []string) pb.Response {
	...
	if queryType != "all" && queryType != "enroll" && queryType != "exchange"{
		return shim.Error(fmt.Sprintf("queryType unknow : %s",queryType))
	}
	...
	for result.HasNext()  {
		...
		if err := json.Unmarshal(historyVal.GetValue(),history);err != nil {
			return shim.Error(fmt.Sprintf("unmarshal history error: %s",err))
		}
		//过滤不是资产转让的记录
...
func (c *AssetExchangeCC) Init(stub shim.ChaincodeStubInterface) pb.Response  {
	return shim.Success(nil)
}
func (c *AssetExchangeCC) Invoke(stub shim.ChaincodeStubInterface) pb.Response{
...
func main() {
	err := shim.Start(new(AssetExchangeCC))
	if err != nil {
